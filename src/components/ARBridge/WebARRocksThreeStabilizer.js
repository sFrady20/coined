...
const WebARRocksThreeStabilizer = (function () {
  ...
  return {
    instance: function (spec) {
      ...
      const _positionSwitchesCount = new Vector3();
      const _positionSigmaFactor = new Vector3(1, 1, 1);
      ...
      function computeQuaternionsMean(quaternions, result) {
        _quaternionSlerps.forEach(function (quaternionSlerpLevel, level) {
          const previousLevelQuats =
            level === 0 ? quaternions : _quaternionSlerps[level - 1];
          quaternionSlerpLevel.forEach(function (quat, i) {
            Quaternion.slerp(
              previousLevelQuats[2 * i],
              previousLevelQuats[2 * i + 1],
              quat,
              0.5
            );
            if (level === _spec.n - 1) {
      ...
      const that = {
        update: function (threePosition, threeQuaternion) {
          ...
          if (_spec.enablePositionSigmaWeighting) {
            computePositionSwitches(
              _lastPositions,
              _positionMean,
              _positionSwitchesCount
            );
            computePositionSigmaFactor(
              _N,
              _positionSwitchesCount,
              _positionSigmaFactor
            );
            _positionSigma.multiply(_positionSigmaFactor);
          }

          const angleSigma = computeQuaternionsSigma(
            _lastQuaternions,
            _quaternionMean
          );
          ...
          const angleStabilized = stabilizeDiffComponent(
            angle,
            angleSigma,
            _spec.k
          );
          _quaternionStabilized
            .copy(_quaternionMean)
            .rotateTowards(threeQuaternion, angleStabilized);
